...
import staticgennan
from staticgennan.utils import ghp_import

log = logging.getLogger(__name__)

default_message = """Deployed {sha} with staticgennan version: {version}"""
...
def _check_version(branch):
    ...
    currentv = parse_version(staticgennan.__version__)
    if not previousv:
        log.warning('Version check skipped: No version specified in previous deployment.')
    elif currentv > previousv:
        log.info(
            'Previous deployment was done with staticgennan version {}; '
            'you are deploying with a newer version ({})'.format(previousv, currentv)
        )
    elif currentv < previousv:
        log.error(
            'Deployment terminated: Previous deployment was made with staticgennan version {}; '
...
def gh_deploy(config, message=None, force=False, ignore_version=False):
    ...
    message = message.format(version=staticgennan.__version__, sha=sha)
