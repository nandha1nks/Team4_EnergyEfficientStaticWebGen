...
def yaml_load(source, loader=yaml.Loader):
    ...
    try:
        ...
        # objects. Some staticgennan internal calls, sometimes in test lib, will
...
def get_themes():
    ...
    themes = {}
    builtins = pkg_resources.get_entry_map(dist='staticgennan', group='staticgennan.themes')

    for theme in pkg_resources.iter_entry_points(group='staticgennan.themes'):

        if theme.name in builtins and theme.dist.key != 'staticgennan':
