...
from staticgennan import utils, theme, plugins
from staticgennan.config.base import Config, ValidationError
...
class Deprecated(BaseConfigOption):
    ...
    def pre_validation(self, config, key_name):
        ...
        warning = ('The configuration option {} has been deprecated and '
                   'will be removed in a future release of staticgennan.'
                   ''.format(key_name))
    ...
    def post_validation(self, config, key_name):
        ...
        if key_name == 'dev_addr' and host in ['0.0.0.0', '::']:
            raise ValidationError(
                ("The staticgennan' server is intended for development purposes only. "
        ...
        if (config['docs_dir'] + os.sep).startswith(config['site_dir'].rstrip(os.sep) + os.sep):
            raise ValidationError(
                ("The 'docs_dir' should not be within the 'site_dir' as this "
                 ...
                 "it will be deleted if --clean is passed to staticgennan build."
                 "(site_dir: '{}', docs_dir: '{}')"
    ...
    def post_validation(self, config, key_name):
        ...
        if key_name == 'pages' and config['pages'] is not None:
            ...
            warning = ("The 'pages' configuration option has been deprecated and will "
                       "be removed in a future release of staticgennan. Use 'nav' instead.")
            self.warnings.append(warning)
