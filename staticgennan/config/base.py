...
log = logging.getLogger('staticgennan.config')
...
class Config(UserDict):
    """
    staticgennan Configuration dict
    ...
    def load_file(self, config_file):
        try:
            ...
            # staticgennan knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "staticgennan encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('staticgennan.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `staticgennan.yml` filename will loaded.
    ...
    from staticgennan import config
